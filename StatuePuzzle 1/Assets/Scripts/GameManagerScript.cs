...
public class GameManagerScript : MonoBehaviour {
    ...
    void applySwaps() {
        int[][,] stateStack = boardStates.ToArray();
        foreach (coord c in swapCoords) {
            int newVal = stateStack[0][c.row, c.col];
            int oldVal = stateStack[1][c.row, c.col];
            if (oldVal == 20 && (newVal == 23 || newVal == 24)) {
                swapAtCoord(c);
            } 
        }
    }

    void swapAtCoord(coord c) {
